...
var WordPasterConfig = {
	...
	//, "PostUrl"			    : "http://www.ncmem.com/AppCode/qiniu/post_gen.aspx"
	, "PostUrl"			    : "http://localhost:81/Joomla_3.4.7/media/wordpaster/upload.php"
    //x86
    ...
    , "ExtensionID"         : "ffminidnhomhklhjhkoiflcepekccfeh"//
...
function debugMsg(m) { jQuery("#msg").append(m);}
...
function WordPasterManager()
{
	...
	this.WordParserNat = {
	    ...
	    {
	        var obj = this;
	        this.plugin = chrome.runtime.connect(_this.Config["ExtensionID"]);
	        this.plugin.onMessage.addListener(function (v)
	        {
	            _this.WordParser_StateChanged(JSON.stringify(v))
	        });
	        //启动exe
	        chrome.runtime.sendMessage(_this.Config["ExtensionID"], { name: "run", app: _this.Config["NatHostName"] });
	        //初始化
	        chrome.runtime.sendMessage(_this.Config["ExtensionID"], {
	            name: "Init"
	            , config: _this.Config
                , fields: _this.Fields
	        });
	        ...
	        {
	            chrome.runtime.sendMessage(_this.Config["ExtensionID"], { name: 'exit' });
	        });
	    }
        , "Paste": function ()
        {
        	if(_this.working) return;
            _this.working = true;
            chrome.runtime.sendMessage(_this.Config["ExtensionID"], {name: "Paste"});
        }
        , "PasteAuto": function (html)
        {
        	if(_this.working) return;
            _this.working = true;
            chrome.runtime.sendMessage(_this.Config["ExtensionID"], { name: "PasteAuto", data: html });
        }
	};
	...
	this.Load = function ()
	{
	    var dom             = jQuery(document.body).append(this.GetHtml());
	...
	{
	    var dom             = jQuery("#" + oid).append(this.GetHtml());
	    this.ffPaster       = dom.find('embed[name="ffPaster"]').get(0);
	    this.ieParser       = dom.find('object[name="ieParser"]').get(0);
	    this.line           = dom.find('div[name="line"]');
	    this.fileItem       = dom.find('div[name="fileItem"]');
